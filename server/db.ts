...
let _unsupportedUrlWarned = false;

export async function getDb() {
  const databaseUrl = process.env.DATABASE_URL;
  if (!_db && databaseUrl) {
    const protocol = (() => { try { return new URL(databaseUrl).protocol; } catch { return ""; } })();
    if (protocol !== "postgres:" && protocol !== "postgresql:") {
      if (!_unsupportedUrlWarned) { console.warn("[Database] PostgreSQL DATABASE_URL is required; incompatible URL scheme ignored"); _unsupportedUrlWarned = true; }
      return null;
    }
    try { _pool = new Pool({ connectionString: databaseUrl }); _db = drizzle(_pool, { schema }); } catch (error) { console.warn("[Database] Failed to initialize PostgreSQL:", error instanceof Error ? error.message : "unknown error"); _db = null; }
  }
  return _db;
}
...
export async function createAsset(input: InsertAsset) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [created] = await db.insert(assets).values(input).returning({ id: assets.id }); return created ? getAssetById(created.id) : undefined; }
...
export async function createRelationship(input: typeof assetRelationships.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [created] = await db.insert(assetRelationships).values(input).returning({ id: assetRelationships.id }); return created ? db.select().from(assetRelationships).where(eq(assetRelationships.id, created.id)).limit(1).then(rows => rows[0]) : undefined; }
...
export async function createNetworkInterface(input: typeof networkInterfaces.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [created] = await db.insert(networkInterfaces).values(input).returning({ id: networkInterfaces.id }); return created ? getNetworkInterfaceById(created.id) : undefined; }
...
export async function createSoftware(input: typeof software.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [created] = await db.insert(software).values(input).returning({ id: software.id }); return created ? getSoftwareById(created.id) : undefined; }
...
export async function createSoftwareInstallation(input: typeof softwareInstallations.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [created] = await db.insert(softwareInstallations).values(input).returning({ id: softwareInstallations.id }); return created ? getSoftwareInstallationById(created.id) : undefined; }
...
export async function createLocation(input: typeof locations.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [created] = await db.insert(locations).values(input).returning({ id: locations.id }); return created ? getLocationById(created.id) : undefined; }
...
export async function createMonitoringTarget(input: typeof monitoringTargets.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [created] = await db.insert(monitoringTargets).values(input).returning({ id: monitoringTargets.id }); return created ? getMonitoringTargetById(created.id) : undefined; }
...
export async function createAlertRule(input: InsertAlertRule) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [created] = await db.insert(alertRules).values(input).returning({ id: alertRules.id }); return created ? getAlertRuleById(created.id) : undefined; }
...
export async function createAlert(input: typeof alerts.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [created] = await db.insert(alerts).values(input).returning({ id: alerts.id }); return created ? getAlertById(created.id) : undefined; }
...
export async function createIncident(input: InsertIncident) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [created] = await db.insert(incidents).values(input).returning({ id: incidents.id }); return created ? getIncidentById(created.id) : undefined; }
