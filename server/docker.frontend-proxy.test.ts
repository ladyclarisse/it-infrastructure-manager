...
describe("production frontend proxy", () => {
  ...
  it("allows the local interface to build without silently attempting an invalid OAuth URL", () => {
    ...
    expect(compose).toContain("VITE_OAUTH_PORTAL_URL: ${VITE_OAUTH_PORTAL_URL:-https://manus.im}");
    expect(oauthClient).toContain("const oauthPortalUrl = (import.meta.env.VITE_OAUTH_PORTAL_URL || \"https://manus.im\").trim();");
